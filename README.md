...
- `特別費`カテゴリの支出は、AIが旅行・年額課金・医療・故障対応などの低頻度支出、または過去の記録漏れ等による差分を合わせるための明示的な調整記録として妥当と判定した場合だけ、通常の予算消化率から除外します。判定不能・不適切なものは予算対象に残ります。
...
週次メールのグラフを、デプロイ・メール送信なしで確認できます。

```bash
npm run preview:budget-chart
```

表示されたURLをブラウザで開くと、月次・週次の通常支出、特別費、予算、分析日を変更して即時に確認できます。PNGとして保存することもできます。
